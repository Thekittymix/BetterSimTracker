...
export function buildMultiCharacterResolverPrompt(input: {
  ...
  return [
    ...
    "- If an entity from the previous scene is still physically co-located, touching, leaning, breathing, sleeping, resting, or otherwise bodily present in the latest active scene frame, keep that entity `inScene=true` even if the latest message mainly advances someone else.",
    "- Physical presence cues are stronger than mere name mentions. A sleeping/resting/silent body in the same interaction space is still `inScene=true`; only mark it absent when the latest context places it away, elsewhere, off-screen, or out of the active frame.",
    ...
    '- If Ashley was in the previous scene and the latest reply says Ashley is asleep, breathing, or leaning against someone in the same bed/room while Blake speaks, keep Ashley as `inScene=true, inMessage=false` and Blake as `inScene=true, inMessage=true`.',
