...
- Fixed extractor failure orchestration so exhausted API/generator retries now abort the active extraction batch instead of continuing into later stat requests before surfacing the failure.
- Strengthened model-backed multi-character resolver guidance so off-screen or elsewhere-mentioned participants are less likely to be treated as active in-scene/message entities just because another character talks about them.
...
- Added explicit target and snapshot guidance blocks to extraction prompts.

### Fixed
- Fixed prompt injection/include behavior so swipe flows use the previous AI snapshot and empty in-scope non-numeric values render as explicit `None`.
- Fixed user-target extraction scoping so active character cards no longer become target-card context during user extraction.
- Fixed sequential custom numeric prompts so `{{statId}}` is resolved in the final prompt instead of leaking literally.
